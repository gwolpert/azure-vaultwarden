...
- **Storage Account**: StorageV2 account hosting an Azure Files share for persistent Vaultwarden data (attachments, Send files, the favicon cache, and the JWT RSA signing keypair). Provisioned with **geo-redundant storage (GRS)** — data is replicated to the Azure paired region for disaster recovery. The public endpoint is **disabled** (`publicNetworkAccess: 'Disabled'`); the App Service reaches the file share over the VNet through a private endpoint in the dedicated `private-endpoints-snet` subnet, with name resolution via the linked `privatelink.file.${environment().suffixes.storage}` private DNS zone. Diagnostic metrics flow to Log Analytics. The container mounts the share at `/data` so that `ATTACHMENTS_FOLDER` (`/data/attachments`), `SENDS_FOLDER` (`/data/sends`), `ICON_CACHE_FOLDER` (`/data/icon_cache`), and `RSA_KEY_FILENAME` (`/data/rsa_key`, written as `rsa_key.pem` / `rsa_key.pub.pem`) are persisted to durable storage. `TEMPLATES_FOLDER` is intentionally left on the local container disk because upstream Vaultwarden requires a local path. `USER_ATTACHMENT_LIMIT` is set to 1000 MiB per user, `ORG_ATTACHMENT_LIMIT` is set to 250 GiB per organization, and `USER_SEND_LIMIT` is set to 100 MiB per user. The file share quota defaults to 250 GB.
