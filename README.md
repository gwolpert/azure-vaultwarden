...
- **Customisable VNet address range**: `vnetAddressPrefixes` (an array of one or more CIDRs), `appServiceSubnetAddressPrefix`, and `postgresqlSubnetAddressPrefix` parameters let you avoid conflicts when peering with other networks. Multiple address prefixes are supported so non-contiguous subnet ranges always fit inside the VNet address space.
